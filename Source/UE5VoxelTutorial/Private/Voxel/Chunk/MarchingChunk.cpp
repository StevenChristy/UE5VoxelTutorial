...
void AMarchingChunk::March(const int X, const int Y, const int Z, const float Cube[8])
{
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		auto V1 = EdgeVertex[TriangleConnectionTable[VertexMask][3 * i]] * BlockSize;
		auto V2 = EdgeVertex[TriangleConnectionTable[VertexMask][3 * i + 1]] * BlockSize;
		auto V3 = EdgeVertex[TriangleConnectionTable[VertexMask][3 * i + 2]] * BlockSize;
